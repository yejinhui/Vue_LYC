<template>
  <div v-if="isAuth(['ROOT', 'ROLE:CHECK'])"
      ...
      <div class="button-item">
        <el-form-item>
          ...
          <el-button type="primary"
                     ...
                     class="button"
                     :disabled="!isAuth(['ROOT', 'ROLE:UPLOAD'])"
                     @click="exportExcel">
          ...
          <el-upload
              ...
              :id="showMore ? 'show-hidden' :''"
              :disabled="importDisabled || !isAuth(['ROOT', 'ROLE:UPLOAD'])"
    ...
    <el-table :data="dataList" style="width: 100%" highlight-current-row
      ...
      <el-table-column prop="roleName" label="角色名称" min-width="150" sortable header-align="center" align="center">
        <template #default="scope">
      ...
      <el-table-column label="权限数量" min-width="150" sortable header-align="center" align="center">
        <template #default="scope">
          <span>{{ scope.row.permissions }}个</span>
        </template>
      </el-table-column>
      <el-table-column prop="users" label="关联用户" min-width="150" sortable header-align="center" align="center">
        <template #default="scope">
          <span>{{ scope.row.users }}个</span>
        </template>
      </el-table-column>
      <el-table-column prop="desc" label="备注" min-width="200" sortable header-align="center" align="center" >
        <template #default="scope">
          <el-popover effect="light" trigger="hover" placement="top" width="auto">
            <template #default>
      ...
      <el-table-column prop="systemic" label="内置角色" min-width="150" sortable header-align="center" align="center">
        <template #default="scope">
      ...
      <el-table-column label="操作" min-width="376" header-align="center" align="center" fixed="right">
        <template #default="scope">
          <el-button size="small"
                     type="info"
                     :disabled="!isAuth(['ROOT', 'ROLE:SELECT']) || scope.row.id===0"
          ...
          <el-button
              size="small"
              type="danger"
              :disabled="!isAuth(['ROOT', 'ROLE:DELETE']) || scope.row.id===0"
...
import selectDetail from "./role-select-detail.vue"
...
export default {
  ...
  methods:{
    ...
    handlerUpload(param) {
      ...
      that.$httpImg('role/uploadRoleExcel', 'POST', formData, true, function (resp) {
        if (resp.result) {
    ...
    printTable(){
      ...
      printJS({
        ...
        properties: [
          ...
          {field:'permissions',displayName:'权限数量'},
          {field:'users',displayName:'关联用户'},
          {field:'desc',displayName:'备注'},
          {field:"systemic",displayName:'内置角色'},
        ],
        scanStyles: false,
        gridHeaderStyle: 'color: red;  border: 2px solid #3971A5;',
        gridStyle: 'border: 2px solid #3971A5;',
        header:'<h3 class="custom">角色管理</h3>',
    ...
    exportAllExcel:function () {
      let that = this;
      that.$http('role/selectAllRoleExcel', 'GET', null, true, function (resp) {
        let list = resp.list;
        that.exportList(list);
      })
    },
    exportList(list){
      let tableData = [
        ['序号','角色名称','权限数量','关联用户','备注','内置角色']//导出表头
      ] // 表格表头
      ...
      XLSX.utils.book_append_sheet(wb, ws, '角色管理') // 工作簿名称
      XLSX.writeFile(wb, '角色管理.xlsx') // 保存的文件名
    },
    //初始化数据
