<template>
  <div v-if="isAuth(['ROOT', 'MODULE:CHECK'])"
      ...
      <div class="button-item">
        <el-form-item>
          ...
          <el-button
              size="small"
              type="success"
              :disabled="!isAuth(['ROOT', 'MODULE:INSERT'])"
              @click="addHandle()"
          >
          ...
          <el-button
              size="small"
              type="danger"
              :disabled="!isAuth(['ROOT', 'MODULE:DELETE'])"
              @click="deleteHandle()"
          >
          ...
          <el-button type="primary"
                     ...
                     class="button"
                     :disabled="!isAuth(['ROOT', 'MODULE:UPLOAD'])"
                     @click="exportExcel">
          ...
          <!--:on-error="onError"和:on-success="onSuccess"和:icon="importDataIcon"-->
          <el-upload
              :show-file-list="false"
              :before-upload="beforeUpload"
              :http-request="handlerUpload"
              :id="showMore ? 'show-hidden' :''"
              :disabled="importDisabled || !isAuth(['ROOT', 'MODULE:UPLOAD'])"
    ...
    <el-table :data="dataList" style="width: 100%" highlight-current-row
      ...
      <el-table-column type="selection" min-width="55" header-align="center" align="center" />
      <el-table-column label="序号" type="index" min-width="70" header-align="center" align="center" fixed>
        <template #default="scope">
          <span>{{ (pageIndex - 1) * pageSize + scope.$index + 1 }}</span>
        </template>
      </el-table-column>
      <el-table-column label="菜单ID" min-width="100" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.id }}
        </template>
      </el-table-column>
      <el-table-column label="菜单编码" min-width="200" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.moduleCode }}
        </template>
      </el-table-column>
      <el-table-column prop="moduleName" label="菜单名称" min-width="120" sortable header-align="center" align="center">
        <template #default="scope">
      ...
      <el-table-column label="链接" min-width="200" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.link }}
        </template>
      </el-table-column>
      <el-table-column label="父ID" min-width="100" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.parentId }}
        </template>
      </el-table-column>
      <el-table-column label="图片" min-width="150" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.iconName }}
        </template>
      </el-table-column>
      <el-table-column prop="status" label="状态" min-width="80" sortable header-align="center" align="center"/>
      <el-table-column prop="defaultModule" label="是否默认菜单" min-width="200" sortable header-align="center" align="center"/>
      <el-table-column label="操作" min-width="250" header-align="center" align="center" fixed="right">
        <template #default="scope">
          <el-button size="small"
                     type="info"
                     :disabled="!isAuth(['ROOT', 'MODULE:SELECT'])"
          ...
          <el-button size="small"
                     type="primary"
                     :disabled="!isAuth(['ROOT', 'MODULE:UPDATE'])"
...
import AddOrUpdate from "./module-add-or-update.vue"
import selectDetail from "./module-select-detail.vue"
...
export default {
  ...
  methods:{
    ...
    handlerUpload(param) {
      ...
      that.$httpImg('module/uploadModuleExcel', 'POST', formData, true, function (resp) {
        if (resp.result) {
    ...
    printTable(){
      ...
      printJS({
        ...
        properties: [
          ...
          {field:'moduleCode',displayName:'菜单编码'},
          {field:'moduleName',displayName:'菜单名称'},
          {field:'link',displayName:'链接'},
          {field:'parentId',displayName:'父ID'},
          {field:'iconName',displayName:'图片'},
          {field:'status',displayName:'状态'},
          {field:'defaultModule',displayName:'是否默认菜单'},
        ],
        scanStyles: false,
        gridHeaderStyle: 'color: red;  border: 2px solid #3971A5;',
        gridStyle: 'border: 2px solid #3971A5;',
        header:'<h3 class="custom">菜单管理</h3>',
    ...
    exportAllExcel:function () {
      let that = this;
      that.$http('module/selectAllModuleExcel', 'GET', null, true, function (resp) {
        let list = resp.list;
        let status = { 0: '无效', 1: '有效' };
        for (let one of list){
          one.status = status[one.status];
        }
        let defaultModule = { 0: '否', 1: '是' };
        for (let one of list){
          one.defaultModule = defaultModule[one.defaultModule];
        }
        that.exportList(list);
      })
    },
    exportList(list){
      let tableData = [
        ['序号','ID', '菜单编码','菜单名称', '链接',"父ID","图片","状态","是否默认菜单"]//导出表头
      ] // 表格表头
      ...
      XLSX.utils.book_append_sheet(wb, ws, '菜单管理') // 工作簿名称
      XLSX.writeFile(wb, '菜单管理.xlsx') // 保存的文件名
    },
    //初始化数据
    loadDataList: function() {
      ...
      that.$http('module/searchModuleByPage', 'POST', data, true, function(resp) {
        let page = resp.page;
        let status = { 0: '无效', 1: '有效' };
    ...
    searchHandle: function() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          if (this.dataForm.moduleName === '') {
            this.dataForm.moduleName = null;
          }
          if (this.pageIndex !== 1) {
            this.pageIndex = 1;
          }
    ...
    selectDetailHandle: function (id) {
      ...
      this.$nextTick(() => {
        this.$refs.selectDetail.init(id);
      });
    },
    //批量删除与单笔删除的方法
    deleteHandle:function(id) {
      ...
      if (ids.length === 0) {
        ...
        that.$confirm(`确定要删除选中的记录？`, '提示', {
          ...
          that.$http('module/deleteModuleByIds', 'POST', {ids: ids}, true, function (resp) {
            if (resp.rows > 0) {
              //只显示一个
