<template>
  <div v-if="isAuth(['ROOT', 'ACTION:CHECK'])"
      ...
      <div class="button-item">
        <el-form-item>
          ...
          <el-button
              size="small"
              type="success"
              :disabled="!isAuth(['ROOT', 'ACTION:INSERT'])"
              @click="addHandle()"
          >
          ...
          <el-button
              size="small"
              type="danger"
              :disabled="!isAuth(['ROOT', 'ACTION:DELETE'])"
              @click="deleteHandle()"
          >
          ...
          <el-button type="primary"
                     ...
                     class="button"
                     :disabled="!isAuth(['ROOT', 'ACTION:UPLOAD'])"
                     @click="exportExcel">
          ...
          <el-upload
              ...
              :id="showMore ? 'show-hidden' :''"
              :disabled="importDisabled || !isAuth(['ROOT', 'ACTION:UPLOAD'])"
    ...
    <el-table :data="dataList" style="width: 100%" highlight-current-row
      ...
      <el-table-column label="功能编码" min-width="250" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.actionCode }}
        </template>
      </el-table-column>
      <el-table-column prop="actionName" label="功能名称" min-width="200" sortable header-align="center" align="center">
        <template #default="scope">
          <el-popover effect="light" trigger="hover" placement="top" width="auto">
            <template #default>
              <div>{{ scope.row.actionName }}</div>
            </template>
            <template #reference>
      ...
      <el-table-column prop="status" label="状态" min-width="120" sortable header-align="center" align="center"/>
      <el-table-column prop="defaultAction" label="是否默认功能" min-width="200" sortable header-align="center" align="center"/>
      <el-table-column label="操作" min-width="376" header-align="center" align="center" fixed="right">
        <template #default="scope">
          <el-button size="small"
                     type="info"
                     :disabled="!isAuth(['ROOT', 'ACTION:SELECT'])"
          ...
          <el-button size="small"
                     type="primary"
                     :disabled="!isAuth(['ROOT', 'ACTION:UPDATE'])"
...
import AddOrUpdate from "./action-add-or-update.vue"
import selectDetail from "./action-select-detail.vue"
...
export default {
  ...
  methods:{
    ...
    handlerUpload(param) {
      ...
      that.$httpImg('action/uploadActionExcel', 'POST', formData, true, function (resp) {
        if (resp.result) {
    ...
    printTable(){
      ...
      printJS({
        ...
        type: 'json',
        properties: [
           // {field:index+1,displayName:'序号'},
          {field:'actionCode',displayName:'功能编码'},
          {field:'actionName',displayName:'功能名称'},
          {field:'status',displayName:'状态'},
          {field:'defaultAction',displayName:'是否默认功能'},
        ],
        scanStyles: false,
        gridHeaderStyle: 'color: red;  border: 2px solid #3971A5;',
        gridStyle: 'border: 2px solid #3971A5;',
        header:'<h3 class="custom">功能管理</h3>',
    ...
    exportAllExcel:function () {
      let that = this;
      that.$http('action/selectAllActionExcel', 'GET', null, true, function (resp) {
        let list = resp.list;
        let status = { 0: '无效', 1: '有效' };
        for (let one of list){
          one.status = status[one.status];
        }
        let defaultAction = { 0: '否', 1: '是' };
        for (let one of list){
          one.defaultAction = defaultAction[one.defaultAction];
        }
        that.exportList(list);
      })
    },
    exportList(list){
      ...
      XLSX.utils.book_append_sheet(wb, ws, '功能管理') // 工作簿名称
      XLSX.writeFile(wb, '功能管理.xlsx') // 保存的文件名
    },
    //初始化数据
    loadDataList: function() {
      ...
      that.$http('action/searchActionByPage', 'POST', data, true, function(resp) {
        let page = resp.page;
        let status = { 0: '无效', 1: '有效' };
    ...
    searchHandle: function() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          if (this.dataForm.actionName === '') {
            this.dataForm.actionName = null;
          }
          if (this.pageIndex !== 1) {
            this.pageIndex = 1;
          }
    ...
    deleteHandle:function(id) {
      ...
      if (ids.length === 0) {
        ...
        that.$confirm(`确定要删除选中的记录？`, '提示', {
          ...
          that.$http('action/deleteActionByIds', 'POST', {ids: ids}, true, function (resp) {
            if (resp.rows > 0) {
              //只显示一个
