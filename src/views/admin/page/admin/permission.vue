<template>
  <div v-if="isAuth(['ROOT', 'ADMIN_PERMISSION:CHECK'])"
    ...
    <el-form :inline="true" :model="dataForm" ref="dataForm" @submit.native.prevent>
      <div class="input-item">
        <el-form-item prop="permissionName">
          <el-input
              v-model="dataForm.permissionName"
              placeholder="权限查询"
              size="small"
              class="input"
              clearable="clearable"
      ...
      <div class="button-item">
        <el-form-item>
          ...
          <el-button
              size="small"
              type="success"
              :disabled="!isAuth(['ROOT', 'ADMIN_PERMISSION:INSERT'])"
              @click="addHandle()"
          >
          ...
          <el-button
              size="small"
              type="danger"
              :disabled="!isAuth(['ROOT', 'ADMIN_PERMISSION:DELETE'])"
              @click="deleteHandle()"
          >
          ...
          <el-button type="primary"
                     ...
                     class="button"
                     :disabled="!isAuth(['ROOT', 'ADMIN_PERMISSION:UPLOAD'])"
                     @click="exportExcel">
          ...
          <el-upload
              ...
              :id="showMore ? 'show-hidden' :''"
              :disabled="importDisabled || !isAuth(['ADMIN_PERMISSION', 'DEPT:UPLOAD'])"
    ...
    <el-table :data="dataList" style="width: 100%" highlight-current-row
      ...
      <el-table-column prop="permissionName" label="权限名称" min-width="300" sortable header-align="center" align="center">
        <template #default="scope">
      ...
      <el-table-column label="授权次数" min-width="150" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.permissions }}
        </template>
      </el-table-column>
      <el-table-column label="操作" min-width="300" header-align="center" align="center" fixed="right">
        <template #default="scope">
          <el-button size="small"
                     type="info"
                     :disabled="!isAuth(['ROOT', 'DEPT:SELECT'])"
          ...
          <el-button size="small"
                     type="primary"
                     :disabled="!isAuth(['ROOT', 'ADMIN_PERMISSION:UPDATE'])"
...
import AddOrUpdate from "./permission-add-or-update.vue"
import selectDetail from "./permission-select-detail.vue"
...
export default {
  ...
  methods:{
    ...
    handlerUpload(param) {
      ...
      that.$httpImg('permission/uploadPermissionExcel', 'POST', formData, true, function (resp) {
        if (resp.result) {
    ...
    printTableHandle(){
      this.printTableTVisible = true;
      this.$nextTick(() => {
        this.$refs.printTableT.init();
      });
    },
    //打印
    printTable(){
      ...
      printJS({
        ...
        properties: [
          ...
          {field:'permissions',displayName:'使用次数'},
        ],
        scanStyles: false,
        gridHeaderStyle: 'color: red;  border: 2px solid #3971A5;',
        gridStyle: 'border: 2px solid #3971A5;',
        header:'<h3 class="custom">权限管理</h3>',
    ...
    exportAllExcel:function () {
      let that = this;
      that.$http('permission/selectAllPermissionExcel', 'GET', null, true, function (resp) {
        let list = resp.list;
        that.exportList(list);
      })
    },
    exportList(list){
      ...
      XLSX.utils.book_append_sheet(wb, ws, '权限管理') // 工作簿名称
      XLSX.writeFile(wb, '权限管理.xlsx') // 保存的文件名
    },
    //初始化数据
    loadDataList: function() {
      ...
      let data = {
        permissionName: that.dataForm.permissionName,
        page: that.pageIndex,
        length: that.pageSize
      };
      that.$http('permission/searchPermissionByPage', 'POST', data, true, function(resp) {
        let page = resp.page;
    ...
    searchHandle: function() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          if (this.dataForm.permissionName === '') {
            this.dataForm.permissionName = null;
          }
          if (this.pageIndex !== 1) {
            this.pageIndex = 1;
          }
    ...
    deleteHandle:function(id) {
      ...
      if (ids.length === 0) {
        ...
        that.$confirm(`确定要删除选中的记录？`, '提示', {
          ...
          that.$http('permission/deletePermissionByIds', 'POST', {ids: ids}, true, function (resp) {
            if (resp.rows > 0) {
              that.$message({
                message: '删除成功',
                type: 'success',
