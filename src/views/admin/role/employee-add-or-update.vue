<template>
  ...
  >
    <el-form :inline="true" :model="dataForm" ref="dataForm" :rules="dataRule" label-width="auto">
      ...
      <el-form-item label="部门" prop="deptId" :required="!dataForm.id">
      ...
      <el-form-item label="公司" prop="companyId" :required="!dataForm.id">
  ...
  data: function () {
    return {
      ...
      dataRule: {
        ...
        deptId: [{required: true,message: '部门必选！'}],
        companyId: [{required: true,message: '公司必选！'}],
      ...
      that.$nextTick(() => {
        ...
        if (id) {
          that.$http('user/searchUserById', 'POST', {id: id}, false, function (resp) {
            ...
            that.dataForm.deptId = params.deptId;
            that.dataForm.companyId = params.companyId;
            that.dataForm.hiredate = params.hiredate;
            that.dataForm.birthday = params.birthday;
            that.dataForm.email = params.email;
            that.dataForm.role = JSON.parse(params.role);
            if (params.status === 1) {
              that.dataForm.status = '在职';
            } else if (params.status === 0) {
              that.dataForm.status = '离职';
            }
          });
        }
        ;
        ...
        that.$http('dept/selectDeptInfoList', 'GET', null, true, function (resp) {
          that.deptNameList = resp.deptNameList
        });
        //加载公司下拉数据
        that.$http('company/selectCompanyInfoList', 'GET', null, true, function (resp) {
          that.companyNameList = resp.companyNameList
        });
    ...
    dataFormSubmit: function () {
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          if (that.dataForm.deptId == null || that.dataForm.deptId === '') {
            data.companyId = null;
          }
          if (that.dataForm.companyId == null || that.dataForm.companyId === '') {
            data.deptId = null;
          }
          if (that.dataForm.status === "在职") {
            data.status = 1;
          }else if(that.dataForm.status === "离职"){
            data.status = 0;
          }
