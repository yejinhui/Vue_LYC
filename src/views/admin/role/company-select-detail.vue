<template>
  <el-dialog
      :title="'公司详情'"
      v-if="isAuth(['ROOT', 'COMPANY:SELECT'])"
  ...
  >
    <el-form :inline="true" :model="dataForm" ref="dataForm" label-width="auto">
      ...
      <el-form-item label="公司英文:" prop="companyEngName">
        <el-text class="el-text-cont">{{ dataForm.companyEngName }}</el-text>
      </el-form-item>
      <el-form-item label="公司地址:" prop="address">
        <el-text class="el-text-cont">{{ dataForm.address }}</el-text>
      </el-form-item>
  ...
  methods: {
    init: function(id) {
      ...
      that.$nextTick(() => {
        ...
        if (id) {
          that.$http('company/searchCompanyById', 'POST', {id : id}, true, function(resp) {
