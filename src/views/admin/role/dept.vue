<template>
  <div v-if="isAuth(['ROOT', 'DEPT:CHECK'])"
    ...
    <el-form :inline="true" :model="dataForm" ref="dataForm" @submit.native.prevent>
      ...
      <!--size的medium是最大号的-->
        ...
        <el-button type="primary"
                   ...
                   class="button"
                   :disabled="!isAuth(['ROOT', 'DEPT:UPLOAD'])"
                   @click="exportExcel">
        ...
        <el-upload
            ...
            :id="showMore ? 'show-hidden' :''"
            :disabled="importDisabled || !isAuth(['ROOT', 'DEPT:UPLOAD'])"
    ...
    <el-table :data="dataList" style="width: 100%" highlight-current-row
      ...
      <el-table-column label="序号" type="index" width="70" header-align="center" align="center" fixed>
      ...
      <el-table-column prop="deptName" label="部门" min-width="100" sortable header-align="center" align="center">
        <template #default="scope">
      ...
      <el-table-column label="电话" min-width="150" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.tel }}
        </template>
      </el-table-column>
      <el-table-column label="电子邮箱" min-width="200" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.email }}
        </template>
      </el-table-column>
    ...
    <print-table-t v-if="printTableTVisible" ref="printTableT"></print-table-t>
  </div>
...
import selectDetail from "./dept-select-detail.vue"
import printTableT from "./print-table-t.vue"
...
export default {
  ...
  methods:{
    ...
    handlerUpload(param) {
      ...
      that.$httpImg('dept/uploadDeptExcel', 'POST', formData, true, function (resp) {
        if (resp.result) {
    ...
    printTable(){
      ...
      printJS({
        ...
        type: 'json',
        properties: [
          {field:'deptName',displayName:'部门名称'},
        ...
        header:'<h3 class="custom">部门管理</h3>',
    ...
    exportAllExcel:function () {
      let that = this;
      that.$http('dept/selectAllDeptExcel', 'GET', null, true, function (resp) {
        let list = resp.list;
    ...
    exportList(list){
      let tableData = [
        ['序号','部门名称', '手机号码','电子邮箱', '描述',"创建日期","状态"]//导出表头
      ] // 表格表头
      ...
      XLSX.utils.book_append_sheet(wb, ws, '部门管理') // 工作簿名称
      XLSX.writeFile(wb, '部门管理.xlsx') // 保存的文件名
    },
    //初始化数据
