<template>
  <div v-if="isAuth(['ROOT', 'USER:CHECK'])"
    ...
    <el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm" @submit.native.prevent>
      ...
      <el-form-item>
        ...
        <el-button type="primary"
                   ...
                   size="small"
                   class="button"
                   :disabled="!isAuth(['ROOT', 'USER:UPLOAD'])"
                   @click="exportExcel">
        ...
        <el-upload
            ...
            :id="showMore ? 'show-hidden' :''"
            :disabled="importDisabled || !isAuth(['ROOT', 'USER:UPLOAD'])"
    ...
    <el-table :data="dataList" style="width: 100%"
      ...
      <el-table-column label="真实姓名" min-width="120" sortable header-align="center" align="center">
        <template #default="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="sex" label="性别"  min-width="100" sortable header-align="center" align="center"/>
      <el-table-column prop="tel" label="联系电话" min-width="130" sortable header-align="center" align="center"/>
      <el-table-column prop="username" label="用户名" min-width="120" sortable header-align="center" align="center"/>
      ...
      <el-table-column label="操作" min-width="355" header-align="center" align="center" fixed="right">
        <template #default="scope">
          <el-button size="small"
                     type="info"
          ...
          <el-button size="small"
                     type="warning"
                     :disabled="!isAuth(['ROOT', 'USER:UPDATE'])"
                     @click="updateUserUpEmployee(scope.row.id)"
                     plain
          >
    ...
    <user-up-employee v-if="userUpEmployeeVisible" ref="userUpEmployee" @refreshDataList="searchHandle"></user-up-employee>
...
import UserUpEmployee from "./user-up-employee.vue"
...
export default {
  ...
  data: function() {
    return {
      ...
      dataRule: {
        name: [{ required: false, pattern: '^[\u4e00-\u9fa5]{2,10}$', message: '姓名格式错误' }],
  ...
  methods:{
    ...
    handlerUpload(param) {
      let that = this;
        ...
        that.$httpImg('user/uploadUserExcel', 'POST', formData, true, function (resp) {
          if (resp.result) {
    ...
    //           new Blob([dataList], {type: 'application/octet-stream'}),
    //           // 设置导出的文件名称可随意
    //           '用户管理.xlsx'
    //       )
    //     } catch (e) {
    ...
    exportExcel:function () {
      let that = this;
      let list = [];
      that.$confirm(`确定要导出选中的记录（没有选中则导出当前列）？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    exportAllExcel:function () {
      let that = this;
      that.$confirm(`确定要导出全部的记录？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        that.$http('user/selectAllUserExcel', 'GET', null, true, function (resp) {
          let list = resp.list;
    ...
    exportList(list){
      let tableData = [
        ['序号','真实姓名', '编号','昵称', '性别',"手机号码","用户名","电子邮箱","生日日期","最近登录状态","创建日期","状态"]//导出表头
      ] // 表格表头
      ...
      XLSX.utils.book_append_sheet(wb, ws, '用户管理') // 工作簿名称
      XLSX.writeFile(wb, '用户管理.xlsx') // 保存的文件名
    },
    //初始化数据
    ...
    //升级员工
    updateUserUpEmployee: function (id) {
      this.userUpEmployeeVisible = true;
      this.$nextTick(() => {
        this.$refs.userUpEmployee.init(id);
      });
    },
    //批量删除与单笔删除的方法
